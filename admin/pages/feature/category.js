...
import { ADD_CATEGORY, DELETE_CATEGORY, EDIT_CATEGORY } from '../../graphql/mutation';
...
const Category = () => {
    ...
    const onFinish = async (values) => {
        try{
            if(edit != null){
                await editCategory({variables: { id: edit.objectId, name: values.name }}).then(async (response) => {
                    form.resetFields();
                    message.success('Category update success');
                    setVisible(false);
                })
            }else{
                await addCategory({ variables: { name: values.name }}).then(async (response) => {
                    form.resetFields();
    ...
    return(
        <div>
            <div className="title" style={{ display: "flex", justifyContent: "space-between", paddingBottom:20}}>
                ...
                <Button onClick={()=> {setVisible(true), form.resetFields(), setEdit(null)}}>Add Category</Button>
            </div>
